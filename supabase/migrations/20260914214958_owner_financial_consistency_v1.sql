...
begin
  ...
  from public.sales s
  where s.shop_id=v_shop and s.status<>'VOID' and s.created_at::date between p_from and p_to;

  select coalesce(sum(r.total_refund),0)
  into v_returns
  from public.sale_return_requests r
  where r.shop_id=v_shop and r.status='APPROVED'
    and coalesce(r.reviewed_at,r.created_at)::date between p_from and p_to;
  ...
  where si.shop_id=v_shop and s.status<>'VOID' and s.created_at::date between p_from and p_to;
  ...
  where rr.shop_id=v_shop and rr.status='APPROVED'
    and coalesce(rr.reviewed_at,rr.created_at)::date between p_from and p_to;
  ...
  where cs.shop_id=v_shop and cs.status='CLOSED' and cs.closed_at::date between p_from and p_to;
  ...
  with days as (
    ...
    select s.created_at::date d,sum(s.grand_total)::numeric amount
    from public.sales s
    where s.shop_id=v_shop and s.status<>'VOID' and s.created_at::date between p_from and p_to
    group by 1
  ), return_day as (
    select coalesce(r.reviewed_at,r.created_at)::date d,sum(r.total_refund)::numeric amount
    from public.sale_return_requests r
    where r.shop_id=v_shop and r.status='APPROVED'
      and coalesce(r.reviewed_at,r.created_at)::date between p_from and p_to
    group by 1
  ...
  with pay as (
    ...
    where p.shop_id=v_shop and p.created_at::date between p_from and p_to
...
create or replace function public.owner_center_summary(
...
begin
  ...
  v:=public.business_analytics(p_from,p_to);
  return v;
end;$$;
...
begin
  ...
  return query
  select s.created_at::date,'Sales'::text,s.invoice_number,
  ...
  return query
  select s.created_at::date,'Sales'::text,s.invoice_number,'Sales Revenue'::text,0::numeric,s.grand_total::numeric,null::text,'WineShopPOS sale','SALE'::text,s.id::text
  from public.sales s
  where s.shop_id=v_shop and s.status<>'VOID' and s.created_at::date between p_from and p_to;

  return query
  select coalesce(rr.reviewed_at,rr.created_at)::date,'Sales Return'::text,
