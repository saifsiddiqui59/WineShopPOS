...
begin
  ...
  from public.sales s
  where s.shop_id=v_shop and s.status<>'VOID'
    and (s.created_at at time zone 'Asia/Kolkata')::date=p_date;

  select coalesce(sum(r.total_refund),0)
  into v_returns
  from public.sale_return_requests r
  where r.shop_id=v_shop and r.status='APPROVED'
    and (coalesce(r.reviewed_at,r.created_at) at time zone 'Asia/Kolkata')::date=p_date;
  ...
  with events as (
    ...
    where si.shop_id=v_shop and s.status<>'VOID'
      and (s.created_at at time zone 'Asia/Kolkata')::date<=p_date
    union all
    ...
    where rr.shop_id=v_shop and rr.status='APPROVED'
      and (coalesce(rr.reviewed_at,rr.created_at) at time zone 'Asia/Kolkata')::date<=p_date
  ), agg as (
