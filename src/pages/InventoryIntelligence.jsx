...
export default function InventoryIntelligence() {
  ...
  async function load(){const{data,error}=await supabase.rpc("inventory_health",{p_history_days:30,p_dead_days:45});if(error)setMessage("Unable to load inventory intelligence.");else{setHealth(data||[]);setMessage("")}}
  useEffect(()=>{void load()},[]);
  ...
  return <div><PageHeader title="Inventory Intelligence" subtitle="Return-adjusted demand, FIFO landed inventory value, health risks and reorder support." tier="PRO"/>{message?<div className="purchase-message">{message}</div>:null}
    <div className="metric-grid four">
      <div className="metric-card metric-accent-green"><span>FIFO Inventory Value</span><strong>{money.format(inventoryValue)}</strong><small>Same landed-cost basis as Dashboard, Reports and Owner</small></div>
      <div className="metric-card metric-accent-orange"><span>Stockout Risk</span><strong>{counts.STOCKOUT_RISK||0}</strong></div>
      <div className="metric-card metric-accent-indigo"><span>Dead Stock</span><strong>{counts.DEAD||0}</strong></div>
      <div className="metric-card metric-accent-blue"><span>Overstock</span><strong>{counts.OVERSTOCK||0}</strong></div>
      <div className="metric-card metric-accent-red"><span>Out of Stock</span><strong>{counts.OUT_OF_STOCK||0}</strong></div>
    </div>
