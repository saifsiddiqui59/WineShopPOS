...
export default function Purchases(){
 ...
 async function barcode(code){
   const i=scannerIndex;setScannerIndex(null);if(i==null)return;
   ...
   if(r.pendingProduct){
     ...
     setMessage(`Barcode ${scan} is kept only in this purchase draft. Product Master is unchanged until Receive Stock succeeds.`);
     return;
   }
   ...
   if(!r.productId){
     ...
     setMessage(`Barcode ${scan} saved in the purchase draft only.`);
     return;
   }
 ...
 async function receive(){if(!navigator.onLine){setMessage("Internet connection required to post inventory. Draft changes remain saved on this device.");return;}if(!ready){setMessage(`Receive Stock blocked: ${firstReview||(!financialReady?"financial reconciliation":!supplierName.trim()?"supplier required":!/^\d{4}-\d{2}-\d{2}$/.test(invoiceDate)?"invoice date required":"review remaining checks")}.`);return;}let ref=invoiceNumber.trim()||`AUTO-${invoiceDate.replaceAll("-","")}-${receiveKey.replace(/[^a-z0-9]/gi,"").slice(-8).toUpperCase()}`,payload={...snapshot(),invoiceNumber:ref,notes:`${notes||""}${notes?"\n":""}[V5-IDEMPOTENCY:${receiveKey}]`};setInvoiceNumber(ref);setBusy(true);setSync("SYNCING");try{await saveOfflinePurchaseDraft(draftId,payload);await saveServer(payload,true);const old=await existing(ref);if(old)return await finishKnown(old,ref);const res=await receiveStock({supplierName:payload.supplierName,invoiceNumber:ref,invoiceDate:payload.invoiceDate,notes:payload.notes,items:payload.items.map(r=>({productId:r.productId||null,pendingProduct:r.pendingProduct||null,sourceDescription:r.sourceDescription||"",invoiceSizeMl:Number(r.invoiceSizeMl||0),scannedBarcode:String(r.scannedBarcode||""),caseCount:Number(r.caseCount),unitsPerCase:Number(r.unitsPerCase),looseBottles:Number(r.looseBottles),quantity:Number(r.quantity),ratePerCase:Number(r.ratePerCase||0),purchasePrice:Number(Number(r.purchasePrice||0).toFixed(6)),mrp:Number(r.mrp||0),lineAmount:Number(r.lineAmount||0),batchNumber:r.batchNumber||"",expiryDate:r.expiryDate||""})),charges:payload.charges});if(!res?.ok){const committed=await existing(ref);if(committed)return await finishKnown(committed,ref);if(!navigator.onLine){setMessage("Connection dropped during Receive. Status UNKNOWN/CHECKING. Do not click Receive again; reconnect first.");return;}throw new Error(res?.message||"Receive failed.");}await link(res.purchaseId);await removeOfflinePurchaseDraft(draftId);await refreshAll();setSync("SYNCED");navigate(`/purchasing/receipts/${res.purchaseId}`);}catch(e){const committed=await existing(ref);if(committed)return await finishKnown(committed,ref);setSync(navigator.onLine?"SYNC ERROR":"OFFLINE");setMessage(e?.message||"Receive failed.");}finally{setBusy(false);}}
 ...
 <section className="panel purchase-receive-footer" style={{marginTop:16}}><div><strong>{ready?"Ready to Receive":"Receive Stock Blocked"}</strong><p className="muted-text">{ready?"All product identity, pack, quantity and financial checks passed.":firstReview||(!financialReady?`Financial reconciliation must match before receiving${difference==null?".":` · difference ${money.format(Math.abs(difference))}.`}`:!supplierName.trim()?"Supplier is required.":!/^\d{4}-\d{2}-\d{2}$/.test(invoiceDate)?"Valid invoice date is required.":"Review remaining purchase checks.")}</p><p className="muted-text">The same Product Master may appear on multiple real invoice rows when commercial values differ. Only identical-looking rows require Keep Separate review.</p><p className="muted-text">Prepared new products and missing barcodes are draft-only until this receive succeeds. A failed or cancelled receive leaves Product Master unchanged.</p></div><button type="button" className="primary-button" disabled={busy||!online||!ready} onClick={receive}>{busy?"Receiving / Verifying...":"Approve & Receive Stock"}</button></section>
 <MobileBarcodeScanner open={scannerIndex!=null} title="Scan Product Barcode" onClose={()=>setScannerIndex(null)} onDetected={barcode}/>
 {createIndex!=null&&createForm?<div className="product-image-chooser-backdrop"><section className="product-image-chooser-modal"><div className="product-image-chooser-header"><div><h3>Prepare New Product</h3><p>Nothing is created yet. This candidate is committed with the purchase only after Approve & Receive Stock succeeds.</p></div><button type="button" className="secondary-button" onClick={()=>setCreateIndex(null)}>×</button></div><div className="form-grid"><label>Product Name<input value={createForm.productName} onChange={e=>setCreateForm(x=>({...x,productName:e.target.value}))}/></label><label>Brand<input value={createForm.brand} onChange={e=>setCreateForm(x=>({...x,brand:e.target.value}))}/></label><label>Category<select value={createForm.categoryId} onChange={e=>setCreateForm(x=>({...x,categoryId:e.target.value}))}><option value="">Uncategorized</option>{activeCategories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Size (ml)<input type="number" min="1" value={createForm.sizeMl} onChange={e=>setCreateForm(x=>({...x,sizeMl:e.target.value}))}/></label><label>Bottles/Case<input type="number" min="1" value={createForm.unitsPerCase} onChange={e=>setCreateForm(x=>({...x,unitsPerCase:e.target.value}))}/></label><label>MRP<input type="number" min="0" value={createForm.mrp} onChange={e=>setCreateForm(x=>({...x,mrp:e.target.value}))}/></label><label>Barcode optional<input value={createForm.barcode} onChange={e=>setCreateForm(x=>({...x,barcode:e.target.value}))}/></label></div><div className="button-row"><button type="button" className="primary-button" disabled={busy} onClick={prepareProduct}>Use on This Purchase</button><button type="button" className="secondary-button" onClick={()=>setCreateIndex(null)}>Cancel</button></div></section></div>:null}
