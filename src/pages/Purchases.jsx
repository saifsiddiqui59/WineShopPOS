...
export default function Purchases(){
 ...
 const ready=Boolean(supplierName.trim())&&/^\d{4}-\d{2}-\d{2}$/.test(invoiceDate)&&!invoiceDateReviewRequired&&items.length>0&&!needsReview&&financialReady;
 const draftId=ingestionId?`ingestion:${ingestionId}`:`manual:${receiveKey}`;
 const snapshot=()=>({version:2,receiveKey,ingestionId:ingestionId||null,supplierId:supplierId||null,supplierName,invoiceNumber,invoiceDate,invoiceDateReviewRequired,notes,items,charges,financialSummary,updatedAt:new Date().toISOString()});
