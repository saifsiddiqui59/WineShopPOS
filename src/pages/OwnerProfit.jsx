...
export default function OwnerProfit(){
  ...
  return <div><PageHeader title="Profit & Business Intelligence" subtitle="Net revenue − COGS = gross profit; gross profit − operating expenses = operating profit." tier="PRO"/>
    ...
    <section className="panel" style={{marginTop:16}}><h3>SKU Profitability</h3>{rows.length===0?<EmptyState title="No profitability data" message="Completed sales with trusted cost data are required."/>:<div className="data-table-wrapper"><SortableTable className="data-table" resizeKey="owner-profit-sku-profitability-v1"><thead><tr><th>Product</th><th>Qty</th><th>Revenue</th><th>COGS</th><th>Gross Profit</th><th>Margin</th></tr></thead><tbody>{rows.map(r=><tr key={r.product_id}><td>{r.product_name}</td><td>{r.quantity}</td><td>{money.format(r.revenue)}</td><td>{money.format(r.cogs)}</td><td><strong>{money.format(r.gross_profit)}</strong></td><td>{Number(r.margin_pct||0).toFixed(2)}%</td></tr>)}</tbody></SortableTable></div>}</section><p className="muted-text">FIFO landed cost is used where available; historical cost snapshots are only a fallback.</p></div>;
}
