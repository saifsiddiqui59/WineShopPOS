...
export default function OwnerCenter() {
  ...
  const chartData = useMemo(() => ({
    ...
    payments:(summary.payment_mix||[]).map(r=>({label:String(r.label||"OTHER"),value:Number(r.value||0)})).filter(r=>r.value!==0),
  ...
  return <div className="dashboard-page">
    ...
    <div className="metric-grid four executive-metrics">
      ...
      <div className="metric-card metric-accent-indigo"><span>Gross Profit</span><strong>{money.format(summary.gross_profit||0)}</strong><small>After FIFO cost of goods</small></div>
      ...
      <div className="metric-card metric-accent-orange"><span>Inventory Cost</span><strong>{money.format(summary.inventory_cost||0)}</strong><small>{chartData.activeProducts} active products · FIFO landed cost</small></div>
    </div>
    ...
    <div className="dashboard-chart-grid primary" style={{marginTop:16}}>
      ...
      <DonutChartCard title="Payment Mix" subtitle="Cash, UPI and Card after refunds" data={chartData.payments} formatValue={(v)=>money.format(v)} centerLabel="Net Sales"/>
    </div>
    <div className="dashboard-chart-grid" style={{marginTop:16}}>
      ...
      <section className="chart-card attention-card">
        ...
        <div className="attention-metrics">
          ...
          <div><span>Total Discount</span><strong>{money2.format(summary.discounts||0)}</strong></div>
        ...
        <p className="muted-text" style={{marginTop:10}}>Sales and profit values already account for discounts. Do not deduct discounts again.</p>
      </section>
    </div>
    ...
    <div className="quick-action-grid"><Link className="quick-action" to="/owner/profit"><strong>Profit Intelligence</strong><span>Net revenue → FIFO COGS → expenses → operating profit</span></Link><Link className="quick-action" to="/inventory/intelligence"><strong>Inventory Health</strong><span>Dead stock, stockout risk and reordering</span></Link><Link className="quick-action" to="/purchasing/intelligence"><strong>Purchase Intelligence</strong><span>OCR, supplier pricing and margin impact</span></Link><Link className="quick-action" to="/owner/share"><strong>Share with Owner</strong><span>Prepare a WhatsApp operating summary</span></Link></div>
  </div>;
}
