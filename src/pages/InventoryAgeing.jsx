...
const boxMark=(lot)=>{const raw=String(lot||"").replace(/[^a-z0-9]/gi,"").toUpperCase();return raw?`BOX-${raw.slice(-6)}`:"-";};
...
export default function InventoryAgeing(){
  ...
  <section className="panel" style={{marginTop:16}}><div className="section-row"><div><h3>True Receipt Ageing</h3><p className="muted-text">Age Priority remains attached to the same product/lot even after sorting. Priority 1 is that product's oldest tracked receipt.</p></div><button className="secondary-button" onClick={load}>Refresh</button></div><div className="data-table-wrapper"><SortableTable className="data-table sticky" showSerial={false} resizeKey="inventory-true-receipt-ageing" defaultColumnWidths={[100,100,240,190,125,125,115,80,90,80,125,125,80]}><thead><tr><th>Product Ref</th><th>Age Priority</th><th>Product</th><th>Supplier</th><th>Box Mark</th><th>Batch</th><th>Receipt Date</th><th>Age</th><th>Bucket</th><th>Qty</th><th>Landed/Bottle</th><th>Cost Value</th><th>%</th></tr></thead><tbody>{ageing.map((r,i)=><tr key={`${r.product_id}-${r.lot_number}-${i}`}><td><strong>{productRef(r.product_id)}</strong></td><td><strong>{ageingPriority.get(priorityKey(r))||"-"}</strong></td><td>{r.product_name}</td><td>{r.supplier_name||"-"}</td><td><strong title={`Technical lot: ${r.lot_number||"-"}`}>{boxMark(r.lot_number)}</strong></td><td>{r.batch_number||"-"}</td><td>{r.receipt_date||"-"}</td><td>{r.age_days==null?"-":`${r.age_days}d`}</td><td>{r.age_bucket}</td><td>{r.quantity}</td><td>{money.format(r.landed_unit_cost||0)}</td><td>{money.format(r.cost_value||0)}</td><td>{Number(r.inventory_percentage||0).toFixed(2)}%</td></tr>)}</tbody></SortableTable></div></section>
  <section className="panel" style={{marginTop:16}}><h3>FIFO Rotation Queue</h3><p className="muted-text">FIFO Priority is stable per product/lot. <strong>1 = SELL FIRST</strong>. Sorting another column changes only the view.</p>{fifo.length===0?<EmptyState title="No tracked receipt lots" message="New V2 purchase receipts create receipt-level lots."/>:<div className="data-table-wrapper"><SortableTable className="data-table sticky" showSerial={false} resizeKey="inventory-fifo-rotation" defaultColumnWidths={[100,100,110,125,250,190,125,115,115,105,125,190]}><thead><tr><th>Product Ref</th><th>FIFO Priority</th><th>Rotation</th><th>Box Mark</th><th>Product</th><th>Technical Lot</th><th>Batch</th><th>Received</th><th>Expiry</th><th>Remaining</th><th>Landed/Bottle</th><th>Supplier</th></tr></thead><tbody>{fifo.map((r,i)=>{const priority=fifoPriority.get(priorityKey(r))||"-";return <tr key={`${r.product_id}-${r.lot_number}-${i}`}><td><strong>{productRef(r.product_id)}</strong></td><td><strong>{priority}</strong></td><td><strong>{priority===1?"SELL FIRST":"NEXT"}</strong></td><td><strong>{boxMark(r.lot_number)}</strong></td><td>{r.product_name}</td><td>{r.lot_number}</td><td>{r.batch_number||"-"}</td><td>{r.received_at?new Date(r.received_at).toLocaleDateString():"-"}</td><td>{r.expiry_date||"-"}</td><td>{r.remaining_quantity}</td><td>{money.format(r.landed_unit_cost||0)}</td><td>{r.supplier_name||"-"}</td></tr>;})}</tbody></SortableTable></div>}</section>
  </>}</div>;
}
