...
export default function PrinterSettings(){
  ...
  const[form,setForm]=useState({store_address:"",store_phone:"",tax_registration_number:"",receipt_footer:"THANK YOU",printer_paper_mm:80});
  const[autoPrint,setAutoPrintState]=useState(false);
  const[message,setMessage]=useState("");
  useEffect(()=>{setAutoPrintState(getReceiptAutoPrint(profile?.shop_id));},[profile?.shop_id]);
  useEffect(()=>{supabase.from("shop_settings").select("store_address,store_phone,tax_registration_number,receipt_footer,printer_paper_mm").maybeSingle().then(({data,error})=>{if(error)setMessage(error.message);else if(data)setForm((current)=>({...current,...data}));});},[]);
  ...
  function toggle(){const next=!autoPrint;setAutoPrintState(next);setReceiptAutoPrint(profile?.shop_id,next);setMessage(`Automatic receipt print ${next?"enabled":"disabled"} on this device.`);}
