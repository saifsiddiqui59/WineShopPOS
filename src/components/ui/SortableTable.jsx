...
function textOf(node){if(node==null||typeof node==="boolean")return"";if(typeof node==="string"||typeof node==="number")return String(node);if(Array.isArray(node))return node.map(textOf).join(" ");if(!isValidElement(node))return"";if(["input","select","textarea"].includes(node.type))return String(node.props?.value??"");return textOf(node.props?.children);}
function comparable(value){const text=String(value||"").replace(/\s+/g," ").trim();const numeric=text.replace(/[₹$€£,%]/g,"").replace(/,/g,"").trim();if(/^[+-]?(?:\d+\.?\d*|\.\d+)$/.test(numeric))return{kind:"number",value:Number(numeric)};if(/^\d{4}-\d{2}-\d{2}/.test(text)||/^\d{1,2}[/-]\d{1,2}[/-]\d{2,4}/.test(text)||/\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b/i.test(text)){const n=Date.parse(text);if(Number.isFinite(n))return{kind:"number",value:n};}return{kind:"text",value:text};}
...
export default function SortableTable({children,showSerial=true,...props}){
  const[sort,setSort]=useState({column:null,direction:"asc"});
  const parts=Children.toArray(children);const hi=parts.findIndex((x)=>isValidElement(x)&&x.type==="thead");const bi=parts.findIndex((x)=>isValidElement(x)&&x.type==="tbody");
