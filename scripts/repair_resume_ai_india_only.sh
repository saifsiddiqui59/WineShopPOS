...
cp "$0" scripts/repair_resume_ai_india_only.sh
chmod +x scripts/repair_resume_ai_india_only.sh

git add docs/ai/AZURE_SUPABASE_CONFIGURATION.md docs/handoff/AI_CLOUD_POLICY_LATEST.txt scripts/repair_resume_ai_india_only.sh
...
ACTUAL_FOUNDRY_LOCATION="$(az cognitiveservices account show --subscription "$SUB_ID" -g "$AZ_RESOURCE_GROUP" -n "$FOUNDRY_ACCOUNT" --query location -o tsv)"
...
FUNCTION_APP_LOCATION="$(az functionapp show -g "$AZ_RESOURCE_GROUP" -n "$FUNCTION_APP" --query location -o tsv | tr '[:upper:] ' '[:lower:]')"
...
git add docs/ai docs/handoff scripts/repair_resume_ai_india_only.sh
