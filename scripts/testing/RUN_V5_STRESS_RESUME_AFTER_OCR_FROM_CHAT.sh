...
REPO="${WSP_REPO:-$CERT_REPO}"
TEST_REL="tests/e2e/v5-stress-resume-after-ocr-from-chat.mjs"
TEST="$REPO/$TEST_REL"
...
[[ -d "$REPO" ]] || die "Missing certification worktree: $REPO"
[[ -f "$RESUME_CASHIER_FILE" ]] || die "Missing preserved cashier credential file: $RESUME_CASHIER_FILE"
cd "$REPO" || die "Missing $REPO"

echo "================================================================"
echo " WineShopPOS V5 — RESUME STRESS AFTER OCR PURCHASE"
echo "================================================================"
echo "Source   : exact prior-chat stress logic"
echo "Flow     : 4 cashiers -> 96+ UI bills -> return -> shifts -> reports/analytics"
echo "R11      : NOT RUN"
echo "Purchase : completed by merged OCR stage before this runner"
echo "PROD     : HARD BLOCKED"
echo "Cashiers : preserved checkpoint $RESUME_CASHIER_FILE"
...
WSP_RESUME_CASHIER_FILE="$(cygpath -w "$RESUME_CASHIER_FILE")" \
  ...
  if ! grep -Fq "$MARKER" "$REGISTER"; then
    {
      echo
      echo "<!-- $MARKER -->"
      echo "## $(date -Iseconds) — STRESS_RESUME_AFTER_OCR"
      echo
      echo "- Runner: \`scripts/testing/RUN_V5_STRESS_RESUME_AFTER_OCR_FROM_CHAT.sh\`"
...
echo
echo "================================================================"
echo " V5 STRESS RESUME AFTER OCR: $([[ "$TEST_RC" -eq 0 ]] && echo PASS || echo FAIL)"
echo " Classification: $CLASSIFICATION"
echo " Git evidence  : $PUBLIC_REL"
echo " Raw evidence  : $RAW_DIR"
echo " Remote V5     : $(git rev-parse origin/V5)"
echo " R11           : NOT RUN"
echo " OCR/Purchase   : PRECEDING MERGED STAGE"
