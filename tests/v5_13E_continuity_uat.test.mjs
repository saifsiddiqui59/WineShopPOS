...
test("Product Image remains selectable and auto-loads in new-product flow",()=>{
...
test("explicit invoice size always wins",()=>{
  assert.equal(inferInvoiceSizeMl({description:"Example CAN 650 ml",unitsPerCaseHint:24},24),650);
  assert.equal(inferInvoiceSizeResolution({description:"Example 50 cl CAN"},24).source,"EXPLICIT_OCR_SIZE");
});
...
test("pack price warning remains under Bottles Case and reviewable",()=>{
...
test("purchase receiving shares size and pack suggestion rules",()=>{
...
test("POS keeps local scanner contract while phone scanner becomes global",()=>{
...
test("continuity doc is sufficient for a new chat",()=>{
  ...
  assert.match(currentState,/41755f157e330cd0eb3cb8667ee6ea063389d25c/);
  ...
  assert.match(currentState,/V5_19/);
