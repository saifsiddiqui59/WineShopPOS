...
const RESUME_CASHIER_FILE = process.env.WSP_RESUME_CASHIER_FILE || "";
...
function loadExistingCashiers(){
  assert(RESUME_CASHIER_FILE && fs.existsSync(RESUME_CASHIER_FILE),
    `Existing cashier credential file is missing: ${RESUME_CASHIER_FILE || "<unset>"}`);
  const saved=JSON.parse(fs.readFileSync(RESUME_CASHIER_FILE,"utf8"));
  const creds=Array.isArray(saved?.users) ? saved.users : [];
  assert(creds.length===4,`Expected exactly 4 saved stress cashiers; found ${creds.length}.`);
  for(const [i,c] of creds.entries()){
    assert(c?.email && c?.password,`Saved cashier ${i+1} is missing email/password.`);
  }
  report.cashiers=creds.map(({password,...x})=>x);
  logPass("Four cashier profiles","reused exact cashier credentials from the preserved stress checkpoint");
  return creds;
}
...
async function finalUiValidation(adminPage,plan,baselineSales){
  ...
  const plannedKeys=new Set([...plan.targets.keys()]);
  const plannedProducts=finalProducts.filter(p=>plannedKeys.has(p.key));
  appAssert(plannedProducts.length===plannedKeys.size,
    `Final Product Master contains ${plannedProducts.length}/${plannedKeys.size} stress-plan products.`);
  const bad=plannedProducts.filter(p=>p.stock<3||p.stock>7);
  appAssert(bad.length===0,`Final stress-product inventory outside 3–7: ${bad.slice(0,10).map(p=>`${p.name} ${p.size}ml=${p.stock}`).join(", ")}`);
  report.stress.finalInventory={
    products:plannedProducts.length,
    min:Math.min(...plannedProducts.map(p=>p.stock)),
    max:Math.max(...plannedProducts.map(p=>p.stock)),
    zeroStockProductsOutsidePlan:finalProducts.filter(p=>!plannedKeys.has(p.key)&&p.stock===0).length,
  };
  logPass("Final stress inventory range",`${plannedProducts.length}/${plannedProducts.length} stress-plan products between 3 and 7 bottles`);
...
try{
  ...
  console.log("============================================================");
  console.log(" V5 STRESS RESUME — EXACT PRIOR-CHAT LOGIC AFTER OCR PURCHASE");
  console.log(" 4 cashiers + 96+ genuine POS UI bills + return + shift close + analytics");
  console.log(" R11 replay: NO · OCR purchase already completed by merged stage · PROD: HARD BLOCKED");
  console.log("============================================================");
