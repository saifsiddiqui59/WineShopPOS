...
rem WineShopPOS Production - one-time Windows app launcher setup.
rem Creates Desktop + Start Menu shortcuts that launch Edge in app mode.
rem No administrator rights required. No browser policies are changed.

set "APP_NAME=WineShopPOS"
set "APP_URL=https://wineshoppos.z29.web.core.windows.net/"
set "EDGE_EXE="
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$s=$ws.CreateShortcut($desktopLink); $s.TargetPath=$edge; $s.Arguments=$args; $s.WorkingDirectory=(Split-Path $edge); $s.IconLocation=$edge + ',0'; $s.Description='WineShopPOS'; $s.Save();" ^
  "$s=$ws.CreateShortcut($startLink); $s.TargetPath=$edge; $s.Arguments=$args; $s.WorkingDirectory=(Split-Path $edge); $s.IconLocation=$edge + ',0'; $s.Description='WineShopPOS'; $s.Save();" ^
...
echo.
echo Setup complete. Opening WineShopPOS now...
